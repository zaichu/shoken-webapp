...
#[cfg(test)] #[rustfmt::skip] mod tests {
    ...
    #[test] fn test_parse_asset_balance_row() {
        ...
        let preview = preview_csv(make_asset_balance_csv(&[INPEX_ROW, NINTENDO_ROW, ACCOUNT_SUMMARY_ROW]).as_bytes()).unwrap(); assert_eq!((preview.total_rows, preview.valid_rows, preview.rows.len(), preview.errors.is_empty()), (2, 2, 2, true), "unexpected errors: {:?}", preview.errors); assert!(matches!(preview_csv(b""), Err(ApiError::ValidationError(_))));
        for (rows, expected_codes) in [(&[TEST_ROW_1, ",,,,,,,,,,,", TEST_ROW_2][..], &["1234", "5678"][..]), (&[INPEX_ROW, NINTENDO_ROW, ACCOUNT_SUMMARY_ROW][..], &["1605", "7974"][..])] { let (items, errors) = parse_asset_balance_csv(make_asset_balance_csv(rows).as_bytes()).unwrap(); assert!(errors.is_empty(), "unexpected errors: {errors:?}"); assert_eq!(items.iter().map(|item| item.security_code.as_str()).collect::<Vec<_>>(), expected_codes); }
    }
}
