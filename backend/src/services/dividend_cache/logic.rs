...
#[cfg(test)] #[rustfmt::skip] mod tests {
    ...
    #[test] fn test_extract_dividend() {
        ...
        for (status, stale_at, expected) in [("pending", None, false), ("error", None, true), ("ok", past, true), ("ok", future, false), ("ok", None, true)] { assert_eq!(compute_is_stale(status, stale_at, now), expected); }
    }
}
