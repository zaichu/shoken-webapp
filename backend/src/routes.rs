...
#[cfg(test)] #[rustfmt::skip] mod tests {
    ...
    async fn assert_rate_limited(router: axum::Router, method: Method, uri: &str, ip: Option<&str>) {
        ...
        let resp = router.clone().oneshot(make_req()).await.unwrap(); assert_ne!(resp.status(), axum::http::StatusCode::TOO_MANY_REQUESTS);
        assert_eq!(router.oneshot(make_req()).await.unwrap().status(), axum::http::StatusCode::TOO_MANY_REQUESTS);
    }
