...
#[cfg(test)] #[rustfmt::skip] mod tests {
    ...
    #[tokio::test] async fn test_routes_rate_limit_returns_429() {
    ...
    #[tokio::test] async fn test_all_endpoints_require_auth() {
    ...
    #[tokio::test] async fn test_request_id_propagated_to_response() {
        ...
        let router: Router = Router::new().route("/health", get(|| async { "OK" })).layer(PropagateRequestIdLayer::x_request_id()).layer(SetRequestIdLayer::x_request_id(MakeRequestUuid)); let health_req = |id: Option<&str>| { let mut b = Request::builder().method(Method::GET).uri("/health"); if let Some(id) = id { b = b.header("x-request-id", id); } b.body(Body::empty()).unwrap() };
        assert!(router.clone().oneshot(health_req(None)).await.unwrap().headers().contains_key("x-request-id"), "x-request-id should be auto-generated"); assert_eq!(router.oneshot(health_req(Some("my-custom-id"))).await.unwrap().headers().get("x-request-id").and_then(|v| v.to_str().ok()), Some("my-custom-id"), "existing x-request-id should be propagated unchanged");
    }
}
