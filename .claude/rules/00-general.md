...
**モデル指定ルール**: `claude` CLI 呼び出し時は原則 `--model` を指定しない。プロジェクト側でモデルを固定せず、Claude CLI やアカウント側のデフォルト/推奨モデル更新に追従しやすくする。ユーザーが明示した場合、または公式に確認した最新/推奨指定がある場合のみ `--model` を指定できる。
